...
void LCD_show(void)
{
		u8 KP[12],KI[12],KD[12],Mood[12],speed[16];			//存放PID参数
	  LCD_Init();
	  POINT_COLOR=RED; //画笔颜色
	  sprintf((char*)KP,"KP:%04f",pid.Kp);//将KP打印到KP数组。
	  sprintf((char*)KI,"KI:%04f",pid.Ki);//将KP打印到KP数组。 
	  sprintf((char*)KD,"KD:%04f",pid.Kd);//将KP打印到KP数组。 
	  sprintf((char*)speed,"speed:%04dr/s",setspeed);//将KP打印到KP数组。
	  sprintf((char*)Mood,"mood:%d",mood);//将KP打印到KP数组。 
	
 	  LCD_Clear(WHITE);
		POINT_COLOR=RED;	  
		LCD_ShowString(30,40,200,16,16,KP); 
		LCD_ShowString(30,60,200,16,16,KI);
		LCD_ShowString(30,80,200,16,16,KD);
 		LCD_ShowString(30,100,200,16,16,speed);		//显示	      					 
		LCD_ShowString(30,120,200,16,16,Mood);	      	
}
